...
function beforeValidate( data, done ) {
  ...
  var _hostname = [];
  var _dockerAddress = process.env.DOCKER_PROXY_DOCKER_ADDRESS || process.env.DOCKER_HOSTNAME || process.env.DOCKER_ADDRESS || 'localhost';

  // Strip out any protocols.
  _dockerAddress = _dockerAddress.replace( 'https://' );
  _dockerAddress = _dockerAddress.replace( 'http://' );
  _dockerAddress = _dockerAddress.replace( 'tcp://' );

  // If appears to have a port (e.g. localhost:2375) we strip it out as well.
  if( _dockerAddress.indexOf( ':' ) ) {
    _dockerAddress = _dockerAddress.split( ':' )[0];
  }
  ...
  if( data.NetworkSettings && data.NetworkSettings.Ports && data.NetworkSettings.Ports ) {
    ...
    Object.keys( data.NetworkSettings.Ports ).forEach( function( key ) {
      ...
      // Replace non-routable address with routable.
      if( [ '0.0.0.0', '127.0.0.1' ].indexOf( _hostIP ) >= 0 ) {
        _hostIP = _dockerAddress;
      }
