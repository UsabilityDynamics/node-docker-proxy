...
getFirstPort = (net) ->
  _.keys(net.Ports)[0].split('/')[0]
makeContainerAddress = (net) ->
  'http://' + net.IPAddress + ':' + getFirstPort net

hostname_key_prefix = 'frontend:'
hostnameKey = (hostname) ->
  hostname_key_prefix + hostname
...
printAllContainers = (cb) ->
  ...
  getAllContainers (err, containers) ->
    console.log padRight(container.ShortId, 16) + padRight(container.Image,
      28) + container.Address for container in containers
    cb()
