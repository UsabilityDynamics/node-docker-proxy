...
function dockerProxy( handler, module ) {
  ...
  this.set({
    ...
    configPath: findup( 'docker-proxy.yaml' ) || require( 'path' ).join( __dirname, '../static/etc/docker-proxy.yaml' ),
    options: {}
  })
  ...
  // this.app.settings = settings.set( this.app.settings ).get();
  // _test.on( 'test.*', console.log);
  // _test.emit( 'test.test', 'blah' );
  // console.log( require( 'util').inspect( _test._events, { colors: true , depth:5, showHidden: false } ) );
...
Object.defineProperties( dockerProxy.prototype, {
  ...
  onMount: {
    ...
    value: function onMount( parent ) {
      this.debug( 'onMount', this.settings.get() );
