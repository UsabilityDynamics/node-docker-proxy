...
function startService( settings ) {
  ...
  auto({
    ...
    settings: [ 'service', function prepareSettings( next, report ) {
      service.log( 'Starting vProxy in [%s] mode.', process.env.NODE_ENV ? 'production' : 'development' );
      ...
      service.set( extend( service.get(), fileExists( settings.parent.config ) ? yaml.readConfig( settings.parent.config ) : {}, {
        ...
        backend: {
          Varnish: {
            ...
            vcl:        service.get( 'backend.Varnish.vcl',         pathJoin( service.get( 'base' ), 'static/etc/varnish/uds.vproxy.vcl' ) ),
  ...
  taskComplete: {
    ...
    value: function taskComplete( error, report ) {
      ...
      if( report.Daemon && report.Daemon.pid ) {
        report.service.log( 'Started vProxy API with [pid=%d].', report.Daemon.pid );
      }

      if( report.Router && report.Router.pid ) {
        report.service.log( 'Started vProxy Router with [pid=%d].', report.Router.pid );
      }

      return this.log( 'The vProxy service was started without issues.' );
