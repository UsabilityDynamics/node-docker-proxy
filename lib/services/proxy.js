...
require( '../docker-proxy' ).service( function serviceHandler( error, service ) {
  ...
  service.once( 'ready', function () {
    ...
    docker  = require( '../clients/docker' ).create( service.get( 'service.api.docker' ) ).on( '_message', handleMessage )
    proxy   = require( 'redbird' )( service.get( 'redbird' ), proxyListening );

    /**
    proxy.on( 'error', function proxyError( error, req, res, data ) {
    ...
    proxy.on( 'proxyReq', function proxyReq(proxyReq, req, res, options) {
      self.debug( 'proxyReq [headers: %d]', Object.keys( req.headers ).length );
    });

    proxy.on( 'proxyRes', function proxyRes(proxyRes, req, res) {
