 ...
 * Veneer Router
 *
 *  supervisor --no-restart-on exit --watch bin,lib --quiet --harmony -- /usr/local/src/vproxy-daemon/lib/services/router
 *  forever start /usr/local/src/vproxy-daemon/lib/services/router/router.js
 *
...
function Middleware( req, res ) {
  ...
  res.setHeader( 'x-via', 'vproxy' );

  if( !options.target ) {
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.write('vProxy - Request can not be proxied, no target found.' + '\n' + JSON.stringify(req.headers, true, 2));
    res.end();
  }
...
function startServers( ) {
  ...
  loadRoutes( process.env.VPROXY_CONFIG_PATH || '/etc/vproxy/vproxy.yaml' );
  ...
  https.createServer( {
    pfx: fs.readFileSync('/etc/ssl/pfx/vproxy.pfx' ),
...
function startService() {
  ...
  module.set({
    config: process.env.VPROXY_CONFIG_PATH || '/etc/vproxy/vproxy.yaml',
