...
require( '../docker-proxy' ).service( function serviceHandler( error, service ) {
  ...
  service.set( 'service.redbird', {
    ...
    redbird: {
      ...
      ssl: {
        port: 8443,
        key: require( 'path' ).join( process.env.DOCKER_PROXY_SSL_DIR_PATH, 'dev-key.pem' ),
        csr: require( 'path' ).join( process.env.DOCKER_PROXY_SSL_DIR_PATH, 'dev-csr.pem' ),
        cert: require( 'path' ).join( process.env.DOCKER_PROXY_SSL_DIR_PATH, 'dev-cert.pem' ),
      }
    }
  });
