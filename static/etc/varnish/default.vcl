...
## UDS Proxy: Varnish
##
## SSH Tunnel setup: http://screencast.com/t/JHlnFahukO51
##
## service nginx restart && service varnish restart
## service nginx configtest
## varnishd -C -f /etc/varnish/default.vcl
##
## New DDP: 172.17.42.1:49155
##
####################################################
...
sub vcl_init {
  ...
  new productionCluster = directors.round_robin();
  productionCluster.add_backend(production);

  new stagingCluster = directors.round_robin();
  stagingCluster.add_backend(staging);
...
sub vcl_recv {
  ...
  if (req.http.X-UDS-Branch == "staging") {
    set req.backend_hint = stagingCluster.backend();
  } else {
    set req.backend_hint = productionCluster.backend();
  }
  ...
  set req.http.Cookie = regsuball(req.http.Cookie, "_icl_current_language=[^;]+(; )?", "");
  set req.http.Cookie = regsuball(req.http.Cookie, "wordpress_test_cookie=[^;]+(; )?", "");
  set req.http.Cookie = regsuball(req.http.Cookie, "PRUM_EPISODES=[^;]+(; )?", "");
  set req.http.Cookie = regsuball(req.http.Cookie, "PRUM_EPISODES=s=[^;]+(; )?", "");
...
set req.http.Cookie = regsuball(req.http.Cookie, "wordpress_test_cookie=[^;]+(; )?", "");
  ...
  #set req.http.Cookie = regsuball(req.http.Cookie, ";(wp-settings-time|wordpress_logged_in_)=", "; \1=");
  #set req.http.Cookie = regsuball(req.http.Cookie, ";[^ ][^;]*", "");
  #set req.http.Cookie = regsuball(req.http.Cookie, "^[; ]+|[; ]+$", "");
...
sub vcl_backend_response {
  ...
  if( ! ( bereq.url ~ "^/manage.*|/wp-admin|/wp-admin.*|/manage/|/manage|/wp-login.php" ) ) {
...
sub vcl_deliver {
   ...
   # If we're an HTTP request, remove browser cachin
  ...
  set resp.http.Via = "vProxy-v2.0.2";
  ...
  unset resp.http.X-Cluster-Version;
  unset resp.http.X-Server;
